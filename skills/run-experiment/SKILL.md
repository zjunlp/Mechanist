---
name: run-experiment
description: Deploy and run ML experiments on local, remote, Vast.ai, or Modal serverless GPU. Use when user says "run experiment", "deploy to server", or needs to launch training jobs.
...
**Parallel runs in one milestone — `nohup ... & wait`:**

```bash
mkdir -p runs/<run_id_A> runs/<run_id_B>
CUDA_VISIBLE_DEVICES=<gpu_A> nohup python <script> --seed 42  > runs/<run_id_A>/run.log 2>&1 &
CUDA_VISIBLE_DEVICES=<gpu_B> nohup python <script> --seed 200 > runs/<run_id_B>/run.log 2>&1 &
wait
```

Shell `wait` joins direct children only — do not use `until ! pgrep -f '<script>.py'; do sleep N; done` (it matches the polling bash's own cmdline → infinite loop). Caller invokes `/monitor-experiment` afterward to finalize each `runs/<run-id>/cost.json`.
...
### Step 5.5: Write initial cost manifest (mandatory, canonical)

Immediately after Step 5 verifies launch, write `runs/<run-id>/cost.json` with what is known at launch time. This file is the **canonical, machine-readable cost record** for the run — `/monitor-experiment` finalizes it on completion (Step 3.6 there), and downstream consumers (`/auto-experiment` Phase 5, `/auto-iteration-loop` Phase C, `/auto-verify`) read from this file rather than parsing free-form deploy logs.

Schema (write all fields; use `null` where the value isn't known yet):

```json
{
  "run_id": "<run-id>",
  "status": "running",
  "started_at": "<ISO 8601 UTC, e.g. 2026-05-18T03:21:00Z>",
  "ended_at": null,
  "gpu_ids": [0, 1, 2, 3],
  "gpu_provider": "remote | local | vast | modal",
...
`gpu_ids` is the effective `CUDA_VISIBLE_DEVICES` list (the leading positional arg parsed in Step 4, post-pinning). For Modal runs where the device list is auto-assigned, write the GPU tier as a single-element array (e.g., `["A100-80GB"]`) and let the finalization step substitute actual numbers from the Modal API.

If `runs/<run-id>/` doesn't exist yet, create it (`mkdir -p`).
