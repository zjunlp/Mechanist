...
**Symptom trigger**: the plan mentions any additive intervention on a representation — steering vector, CAA, DAS interchange dose-response, representation engineering, SAE feature scaling, ROME-style edit — with a configurable strength parameter (`α`, `β`, `dose`, `magnitude`, `scale`, `coefficient`, `k`). Fires on "steering had no effect", "random direction beat my steering vector", "model output garbage after steering" — **and it fires just as hard when the plan already fixes a coefficient or copies one from a reference paper: a fixed coefficient is a grid point to be swept, not a reason to skip the sweep**.
...
**What the tip provides**: five rules — (1) always sweep the coefficient over a wide range; (2) express the coefficient in σ units (`σ = std(hᵀu)`, the projection std at the intervention site) and sweep in σ, so the grid is comparable across layers / directions / models; (3) the **only** acceptance test is the pass criteria in `task.md` or the relevant milestone's claim criteria — a clean dose-response curve, a large effect size, a fluent-looking sample certify nothing; (4) criteria not met → widen the range and re-run, and touch other knobs only after the whole range has failed; (5) stopping with no passing coefficient is not a negative finding — it **requires** an `open_items[]` warning that the range may not have been swept widely enough.
