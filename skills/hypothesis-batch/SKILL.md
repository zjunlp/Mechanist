...
**The search step is mandatory and may not be collapsed into a prior global survey.** P1's `LANDSCAPE.md` is background context only. It never substitutes for the candidate's own `SearchLiterature` action. Every candidate must execute its own two-source search loop — `/mechanic-db-search` first, then `WebSearch` — before that candidate is allowed to finalize.
...
- **No parallel, background, or batch candidate generation of any kind.** Do not launch multiple candidate writers and wait for them together. Do not pre-write a batch script, pre-materialize many candidate JSONs, or append many ideas in one step. The only allowed control flow is: search for candidate `i` -> think/refine candidate `i` -> finalize candidate `i` -> append/flush candidate `i` -> start candidate `i+1`.
...
- **The search is not optional, skippable, or deferrable.** Never skip the candidate-level search because of time, quota anxiety, impatience, or because P1 already found "enough" papers. Never claim that per-candidate search is infeasible and proceed anyway. If the candidate has not completed its own `SearchLiterature` action, it is not eligible to finalize.
- **"Search" means both sources, in order, every time.** A valid candidate-level search consists of `/mechanic-db-search` followed by `WebSearch`, then result merging and de-duplication. Running only one source, or replacing them with memory/prior knowledge/LANDSCAPE-only reasoning, is a protocol violation.
...
- **Each candidate must receive real thought time after the search.** Do not treat P2 as a throughput benchmark. After the candidate's search results are in hand, the writer must spend enough deliberate reasoning on novelty, distinction from related work, experimental tractability, and divergence from `prev_ideas_string` before finalizing. Fast batch-like drafting, copy-pattern generation, or superficial paraphrase of previous ideas is a failure mode, not an optimization.
- **Do not trade away search depth for speed.** The writer has ample time to run the required search and think carefully. "Too many candidates", "the search is slow", or "I already know the literature" are never acceptable excuses for skipping `/mechanic-db-search`, skipping `WebSearch`, or rushing to `FinalizeIdea`.
