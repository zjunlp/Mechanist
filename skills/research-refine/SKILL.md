...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (external LLM reviewer via llm-chat): Review for fidelity, specificity, contribution quality, and frontier leverage
  -> Phase 3 (Claude): Anchor check + simplicity check -> revise method -> rewrite full proposal
  -> Phase 4 (external LLM reviewer, fresh call with prior context in prompt): Re-evaluate revised proposal
...
Check both paper channels first — `literature/` (user-curated) and `papers/` (auto-downloaded). Read only the relevant parts needed to answer:
...
Send the full proposal to the external LLM reviewer for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu. Always ask the external reviewer for strict, high-rigor feedback.
...
**CRITICAL: Save the FULL raw response** verbatim. `llm-chat` is stateless — the full raw response IS the record. Later rounds must include a verbatim summary of the prior round's review in the prompt.
...
Send the revised proposal to the external LLM reviewer via a **fresh llm-chat call**. Because llm-chat is stateless, include a verbatim summary of the prior round's review (scores, verdict, top action items) inside the prompt:

```
mcp__llm-chat__chat:
  prompt: |
    [Round N re-evaluation — llm-chat is stateless; prior context is included below]

    ## Prior Round Review (verbatim summary)
    - Previous overall score: X/10
    - Previous verdict: [READY / REVISE / RETHINK]
    - Previous top action items / drift warnings:
      [paste verbatim from prior round review]

    I revised the proposal based on that feedback.
...
[Full verbatim response from the external LLM reviewer]
...
- **llm-chat is stateless.** Every round must include the prior review summary in the prompt for the reviewer to stay consistent.
...
/auto-iteration-loop               -> iterate on results and paper
```
