...
- **Dataset / split / task** — also state **data provenance** (`existing` = an existing dataset used as-is, `adapted` = an existing dataset transformed/relabeled/filtered, `constructed` = built from scratch / synthetic), the **source** (dataset name, or construction method when `constructed`), the **total available size** of that source (`available_n`), and the **planned amount this project will actually use** (`used_n`). When `used_n < available_n`, state how/why it is subsetted. **Budget-gate the subset:** subset **only** when a scientific reason applies (a ladder-of-evidence screen, or the full set genuinely exceeds the declared GPU / compute budget) — do **not** subset merely to save cost when `task.md` declares a budget that covers the full `available_n`; in that case plan `used_n = available_n` at the user's named model size. A declared, generous budget is a mandate to run at fuller scale, not just an upper bound (see `/auto-experiment` GPU-budget rule).
...
- Dataset / split / task:
  ...
  - Source (name, or construction method if constructed):
  - Available N (total in source pool):
  - Planned used N (this project):
  - Subset note (only if planned used N < available N):
- Compared systems:
- Metrics:
- Setup details:
- Success criterion:
...
/auto-iteration-loop  -> react to results and iterate on the paper
```
